...
function prepend(dob,list) {
    list = {value: dob, rest: list};
    return list;
}
...
console.log(`Ten`);
sp=[[1,11],[2,22],[3,33]]
console.log(sp.reduce(function(a, b) {
    return a.concat(b);
}, []));


console.log(`Eleven`);
var bisls_FILE = JSON.stringify([
...
bisls = JSON.parse(bisls_FILE);
function average(array) {
    function plus(a, b) { return a + b; }
    return array.reduce(plus) / array.length;
}
byName = {};
bisls.forEach(function(person) {
    byName[person.name] = person;
});
raz = bisls.filter(function(metes) {
...
console.log(`Twelve`);
var bisls = JSON.parse(bisls_FILE);
function average(array) {
    function plus(a, b) { return a + b; }
    return array.reduce(plus) / array.length;
}
